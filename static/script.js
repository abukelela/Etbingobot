...
async function loadDepositAccounts() {
  ...
  try {
    ...
    for (var i = 0; i < keys.length; i++) {
      ...
      btn.onclick = (function(k) { return function() { selectMethod(k); }; })(key);
...
function copyAccount() {
  ...
  if (navigator.clipboard) {
    navigator.clipboard.writeText(info.textContent).then(function() { showToast('✅ ተቀድቷል!'); });
  }
}
...
async function submitDeposit() {
  ...
  try {
    var r = await fetch('/api/deposit/request', {
      method: 'POST',
      headers: { 'Content-Type': 'application/json' },
      body: JSON.stringify({ user: userId, name: userName, amount: amount, method: depSelectedMethod, reference: ref })
    });
    var data = await r.json();
    if (data.ok) {
      showToast('✅ ጥያቄዎ ተልኳል!');
  ...
  finally { btn.disabled = false; btn.textContent = '✅ ላክ'; }
}
...
async function submitWithdraw() {
  ...
  try {
    var r = await fetch('/api/withdraw/request', {
      method: 'POST',
      headers: { 'Content-Type': 'application/json' },
      body: JSON.stringify({ user: userId, name: userName, amount: amount, method: wdSelectedMethod, account: account })
    });
    var data = await r.json();
    if (data.ok) {
      showToast('✅ ጥያቄዎ ተልኳል!');
      closeWithdraw();
      await fetchBalance();
    } else { showToast('⚠️ ' + (data.error || 'ስህተት')); }
  } catch (e) { showToast('⚠️ የኢንተርኔት ችግር'); }
  finally { btn.disabled = false; btn.textContent = '✅ ጥያቄ ላክ'; }
}
...
async function showHistory() {
  ...
  try {
    ...
    for (var i = 0; i < txs.length; i++) {
      ...
      var date = tx.created_at ? new Date(tx.created_at).toLocaleString('am-ET', { month: 'short', day: 'numeric', hour: '2-digit', minute: '2-digit' }) : '';
      div.innerHTML = '<div class="tx-icon">' + icon + '</div>' +
        '<div class="tx-info"><div class="tx-label">' + label + '</div><div class="tx-date">' + date + '</div></div>' +
        '<div class="tx-amount">' + (isPositive ? '+' : '') + tx.amount.toFixed(2) + '</div>';
      list.appendChild(div);
    }
  } catch (e) { list.innerHTML = '<div class="tx-empty">⚠️ ስህተት</div>'; }
}
...
function inviteFriends() {
  ...
  if (window.Telegram && window.Telegram.WebApp) {
    ...
    try {
      var shareUrl = 'https://t.me/share/url?url=' + encodeURIComponent(roomUrl) + '&text=' + encodeURIComponent(inviteText);
  ...
  } else { copyToClipboard(inviteText + '\n\n' + roomUrl); }
}

function copyToClipboard(text) {
  if (navigator.clipboard) {
    navigator.clipboard.writeText(text).then(function() { showToast('✅ ሊንኩ ተቀድቷል!'); });
  }
}
...
async function joinGame(cardNum) {
  ...
  try {
    ...
    var data = await r.json();
    if (data.error) {
      if (data.error === 'insufficient_balance') { showToast('💰 ሂሳብ አይበቃም!'); showDeposit(); }
      else showToast('⚠️ ' + data.error);
      return;
    }
    showToast('✅ ተቀላቅለሃል!');
    if (data.balance !== undefined) { userBalance = data.balance; updateBalanceDisplay(); }
    await fetchState();
  } catch (e) { showToast('⚠️ የኢንተርኔት ችግር'); }
}
...
function renderPicker() {
  ...
  for (var i = 1; i <= 144; i++) {
    ...
    cell.onclick = (function(n) { return function() { closePicker(); joinGame(n); }; })(i);
    grid.appendChild(cell);
  }
}

function confirmNewGame() { if (confirm('አዲስ ጨዋታ ጀምር?')) createGame(); }
